...
@app.get("/simulator/v2v5-early-capture-replay-ultra")
@app.get("/simulator/v2v5-ultra")
...
def live_hunting_replay_endpoint(
    ...
    runtime_note = {}
    path_lower = str(getattr(getattr(request, "url", None), "path", "") or "").lower()
    ultra_runtime_mode = ("v2v5-ultra" in path_lower) or ("early-capture-replay-ultra" in path_lower)
    ...
    if bool(safe_runtime_mode) or ultra_runtime_mode:
        before = {
            ...
            "ultra_runtime_mode": ultra_runtime_mode,
        }
        if ultra_runtime_mode:
            # V2V5c: very small replay that must return before Railway proxy timeout.
            # It is for validating whether Early Capture Boost logic works, not for a full-day score.
            eff_max_prepared = max(10, min(eff_max_prepared, 25))
            eff_max_symbols = max(10, min(eff_max_symbols, 25))
            eff_prior_scan_max_rows = max(120000, min(eff_prior_scan_max_rows, 450000))
            eff_max_minute_rows = max(150000, min(eff_max_minute_rows, 450000))
            eff_prior_scan_timeout_sec = max(5.0, min(eff_prior_scan_timeout_sec, 9.0))
        else:
            eff_max_prepared = max(20, min(eff_max_prepared, 50))
            eff_max_symbols = max(20, min(eff_max_symbols, 45))
            eff_prior_scan_max_rows = max(250000, min(eff_prior_scan_max_rows, 900000))
            eff_max_minute_rows = max(250000, min(eff_max_minute_rows, 800000))
            eff_prior_scan_timeout_sec = max(6.0, min(eff_prior_scan_timeout_sec, 14.0))
        eff_include_candidates = False
        runtime_note = {
            "v2v5c_safe_runtime_mode": True,
            "ultra_runtime_mode": bool(ultra_runtime_mode),
            "before": before,
            "effective": {
                "max_prepared": eff_max_prepared,
                "max_symbols": eff_max_symbols,
            ...
            "rule_ar": "V2V5c: تقليل حاد لحدود المحاكي حتى لا يسقط Railway. استخدم ultra أولًا للتأكد من المنطق، ثم safe، ثم الكامل عند الحاجة.",
        }
    try:
        ...
        if isinstance(payload, dict) and runtime_note:
            payload["runtime_guard_v2v5c"] = runtime_note
            payload["version"] = str(payload.get("version") or "") + "+v2v5c_runtime_guard"
    except Exception as exc:
        payload = {
            "ok": False,
            "version": "v2v5c_runtime_safe_endpoint_guard_2026_06_21",
            "error": f"live_hunting_replay_exception:{type(exc).__name__}:{str(exc)[:240]}",
            "rule_ar": "V2V5c: إذا ظهر upstream error حتى مع safe فابدأ بمسار ultra؛ هذا لا يغير قرارات الشراء بل يقلل حجم المحاكاة فقط.",
        }
...
def _pattern_wf_params(
    ...
    return {
        ...
        "max_rows_per_day": max(50000, min(650000, int(max_rows_per_day or 450000))),
    }


def _pattern_wf_job_id(params: dict) -> str:
    raw = json.dumps(params or {}, sort_keys=True, ensure_ascii=False) + str(time.time())
    return "wf_" + hashlib.sha1(raw.encode("utf-8")).hexdigest()[:12]


def _pattern_wf_background_worker(job_id: str, params: dict) -> None:
    started = _pattern_wf_now_label()
    payload = {
        "ok": True,
        "running": True,
        "job_id": job_id,
    ...
    try:
        ...
        pass
    try:
        result = run_gpt_pattern_walk_forward_replay(**params)
        result["async_job"] = {
            "job_id": job_id,
            "started_at": started,
        ...
        final_payload = {
            ...
            "job_id": job_id,
            "started_at": started,
            "finished_at": result["async_job"]["finished_at"],
            "params": params,
            "result": result,
        }
        try:
        ...
        err_payload = {
            ...
            "finished_at": _pattern_wf_now_label(),
            "params": params,
            "error": "walk_forward_background_failed",
...
def pattern_lab_gpt_walk_forward_status_endpoint():
    ...
    return {
        ...
        "finished_at": state.get("finished_at") or saved.get("finished_at") or "",
        "params": state.get("params") or saved.get("params") or {},
        "last_job": saved,
        "has_result": bool(result),
        "result_preview": {
            "ok": bool(result.get("ok")) if isinstance(result, dict) else False,
            "version": result.get("version") if isinstance(result, dict) else None,
            "learn_dates": result.get("learn_dates") if isinstance(result, dict) else None,
            "test_dates": result.get("test_dates") if isinstance(result, dict) else None,
            "missing_dates": result.get("missing_dates") if isinstance(result, dict) else None,
            "forward_signals_count": ((result.get("forward_period") or {}).get("signals_count") if isinstance(result, dict) else None),
        },
        "result_url": "/pattern-lab/gpt/walk-forward/result",
        "message_ar": "إذا running=true انتظر ثم حدّث. إذا has_result=true افتح result أو انسخ forward_period الملخص.",
    }
...
def pattern_lab_gpt_walk_forward_result_endpoint(compact: bool = True):
    ...
    return {
        ...
        "walk_forward_version": result.get("walk_forward_version"),
        "learn_dates": result.get("learn_dates"),
        "test_dates": result.get("test_dates"),
        "missing_dates": result.get("missing_dates"),
        "source_used": result.get("source_used"),
        "polygon_pull_status": result.get("polygon_pull_status"),
        "forward_period": result.get("forward_period"),
        "recommended_next_step_ar": result.get("recommended_next_step_ar"),
        "rule_ar": result.get("rule_ar"),
        "async_job": result.get("async_job"),
    }
...
def pattern_lab_gpt_walk_forward_endpoint(
    ...
    params = _pattern_wf_params(
        start_date=start_date,
        learn_days=learn_days,
        test_days=test_days,
        limit_symbols=limit_symbols,
        horizon_bars=horizon_bars,
        use_polygon_flatfiles=use_polygon_flatfiles,
        force_polygon_pull=force_polygon_pull,
        max_rows_per_day=max_rows_per_day,
    )
    if bool(sync):
        return run_gpt_pattern_walk_forward_replay(**params)
    return _pattern_wf_start_async(params)
