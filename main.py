...
def prepared_explosion_watch_endpoint(format: str = "json"):
    ...
    payload = {
        "ok": True,
        "version": "prepared_explosion_watch_status_v2u3_critical_promotion_gate_2026_06_20",
        "count": len(items or []),
    ...
    if str(format or "json").lower() in {"brief", "text", "txt"}:
        lines = ["Prepared Explosion Watch V2U3", f"count: {payload['count']}", "symbols: " + ", ".join(payload["symbols"][:80]), str(debug)]
...
def prior_session_explosion_scan_endpoint(
    ...
    if str(format or "json").lower() in {"brief", "text", "txt"}:
        ...
        lines = [
            "Prior Session Explosion Scan V2U3",
            f"date: {trade_date}",
            f"ok: {payload.get('ok')}",
