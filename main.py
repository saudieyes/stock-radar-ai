...
PERFORMANCE_STORE_LOCK = threading.Lock()
...
def scan_all():
    ...
    # تسجيل التتبع الأسبوعي بشكل تسلسلي بعد انتهاء الفحص كله لتجنب ضياع بعض الإشارات عند الكتابة المتزامنة.
    for item in rows:
        try:
