...
BIG_EXPLOSION_PREPARED_WATCH_LIMIT = _env_int("BIG_EXPLOSION_PREPARED_WATCH_LIMIT", 320, 40, 600)
...
def save_prepared_big_explosion_watch(items: list[dict] | None, trade_date: str = "", source: str = "", debug: dict | None = None) -> dict:
    ...
    for it in (items or []):
        ...
        clean.append({
            ...
            "metrics": {
                ...
                if k in {
                    ...
                    "prepared_bucket", "prepared_bucket_ar", "pre_explosion_candidate_v2u1",
                    "opening_gap_candidate_v2u1", "ultra_low_price_candidate_v2u1",
                    "after_hours_pressure_v2u1", "quiet_pressure_v2u1", "true_mining_score_v2u1",
                    "watch_priority_v2u1", "blocked_but_watchlist_v2u1"
                }
            },
        })
        if len(clean) >= int(BIG_EXPLOSION_PREPARED_WATCH_LIMIT or 160):
            break
    payload = {
        "version": "big_explosion_prepared_watch_v2u1_true_mining_2026_06_20",
        "updated_at_utc": now,
        ...
        "items": clean,
        "debug": dict(debug or {}),
        "rule_ar": "V2U1: قائمة جاهزة من مسح أمس الكامل مع تعدين مرشحي ما قبل الانفجار؛ تمر عبر الشرعية والتحليل العميق ولا تعني شراء مباشر.",
    }
    ok = False
    try:
...
def load_prepared_big_explosion_watch() -> tuple[list[dict], dict]:
    ...
    try:
        ...
        payload = {}
    debug = {
        "version": "prepared_big_explosion_watch_loader_v2u1_true_mining_2026_06_20",
...
def build_dynamic_universe(max_symbols: int = 700) -> list[str]:
    ...
    diag = {
        "engine_version": "dynamic_discovery_v3i1_true_pre_explosion_mining_v2u1_2026_06_20",
        "dynamic_discovery_enabled": True,
        "dynamic_discovery_mode": "real_pre_explosion_capture_v2u1_true_mining_prepared_watch_plus_opening_instant",
