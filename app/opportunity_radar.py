...
OPPORTUNITY_RADAR_VERSION = "opportunity_radar_rebuild_v2u1_true_pre_explosion_mining_2026_06_20"
...
def _big_explosion_live_profile(row: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "rule_ar": "V2U1: مسار تعدين ومراقبة ما قبل الانفجار وقائمة أمس الجاهزة؛ لا يغيّر Strong/Cautious ولا يعني دخول مباشر.",
    }
...
def _prep_candidate_sections(row: dict) -> list[tuple[str, float, list[str]]]:
    ...
    if big_explosion.get("matched"):
        if row.get("big_explosion_prepared_watch_v2u"):
            reasons = ["V2U1: مرشح انفجار جاهز قبل السوق من تعدين جلسة أمس الكامل — راجع الشرعية مبكرًا وراقب البري ماركت/الافتتاح."]
