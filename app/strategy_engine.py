    ...
    try:
        ...
        if price <= 0 or len(highs) < 10 or len(lows) < 10:
            return {
                ...
                "support_resistance_version": "v4h_practical_nearest_level_guard",
                "price_discovery_zone": False,
        ...
        resistance_levels_above_raw = _unique_levels(resistance_candidates, reverse=False)

        nearest_support = max(support_candidates) if support_candidates else 0.0
        raw_nearest_resistance = min(resistance_candidates) if resistance_candidates else 0.0
        raw_resistance_dist = ((raw_nearest_resistance - price) / price) * 100 if raw_nearest_resistance > 0 and price > 0 else 0.0

        # V4h sanity: a very distant historical high is not a practical nearest
        # resistance. Keep it as a major/historical level, but do not let it
        # drive "near resistance" or 52w-high labels.
        PRACTICAL_RESISTANCE_MAX_DIST_PCT = 22.0
        nearest_resistance = raw_nearest_resistance if (raw_nearest_resistance > 0 and raw_resistance_dist <= PRACTICAL_RESISTANCE_MAX_DIST_PCT) else 0.0
        resistance_levels_above = [x for x in resistance_levels_above_raw if price > 0 and ((float(x) - price) / price) * 100 <= PRACTICAL_RESISTANCE_MAX_DIST_PCT]

        # Major resistance: next 52w/ATH level above price if available, even if far.
        year_high = float(hist.get("year_high", 0) or 0)
        ath_high = float(hist.get("ath_high", 0) or 0)
        recent_120_high = max(highs[-120:]) if highs else 0
        major_res_candidates = [x for x in [year_high, ath_high, recent_120_high, raw_nearest_resistance] if x > price * 1.003]
        major_resistance = min(major_res_candidates) if major_res_candidates else 0.0
        ...
        price_discovery_zone = bool(nearest_resistance <= 0 and (year_high <= 0 or price >= year_high * 0.985 or price >= recent_120_high * 0.985))
        ...
        elif price_discovery_zone:
            resistance_label = "لا توجد مقاومة عملية قريبة — منطقة اكتشاف سعر/اختراق قمة حديثة"
        else:
        ...
        elif year_high > 0 and price >= year_high * 0.985:
            major_label = "قرب/اختراق هاي 52 أسبوع"
        elif major_resistance > 0:
            dist_major = ((major_resistance - price) / price) * 100 if price > 0 else 0
            if dist_major > PRACTICAL_RESISTANCE_MAX_DIST_PCT:
                major_label = f"مقاومة تاريخية بعيدة قرب {safe_round(major_resistance)} (ليست مقاومة قريبة)"
            else:
        ...
        return {
            ...
            "support_resistance_version": "v4h_practical_nearest_level_guard",
            "price_discovery_zone": bool(price_discovery_zone),
            "raw_nearest_resistance": safe_round(raw_nearest_resistance),
            "raw_nearest_resistance_distance_pct": safe_round(raw_resistance_dist, 2) if raw_nearest_resistance else 0.0,
            "active_support": safe_round(nearest_support),
            "active_resistance": safe_round(nearest_resistance),
            "major_resistance_is_distant": bool(major_resistance > 0 and price > 0 and ((major_resistance - price) / price) * 100 > PRACTICAL_RESISTANCE_MAX_DIST_PCT),
